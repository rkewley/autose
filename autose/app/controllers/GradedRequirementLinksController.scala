...
object GradedRequirementLinksController extends ControllerTrait[Long, MdlGradedRequirementLinks, Long] with Base {
  ...
  val form = Form[MdlGradedRequirementLinks](
	...
	"fGradedRequirement" -> of[Long]
    )(MdlGradedRequirementLinks.apply)(MdlGradedRequirementLinks.unapply)
  )
  ...
  def uploadGradedEventFile(gradedEventId: Long) = compositeAction(NormalUser) { user => implicit template => implicit request =>
    val vGradedEventLinks = new MdlGradedRequirementLinks(Option(0), "http:/", "", true, gradedEventId)
      ...
      vGradedRequirementLinks => {
        request.body.file("gradedRequirementFile").map { gradedRequirementFile =>
          if (vGradedRequirementLinks.validate) {
            ...
            val course = SqlCourses.select(vGradedRequirements.vCourse)
